...
void util_shortcut_undo_redo() {
	...
	bool redo_pressed = keymap_shortcut(any_map_get(g_keymap, "edit_redo"), SHORTCUT_TYPE_STARTED) || (keyboard_down("control") && keyboard_started("y"));
...
void util_shortcut_global() {
	if (!g_ui->is_typing) {
		if (keymap_shortcut(any_map_get(g_keymap, "toggle_node_editor"), SHORTCUT_TYPE_STARTED)) {
			ui_nodes_canvas_type == CANVAS_TYPE_MATERIAL ? ui_base_show_material_nodes() : ui_base_show_brush_nodes();
		}
		else if (keymap_shortcut(any_map_get(g_keymap, "toggle_browser"), SHORTCUT_TYPE_STARTED)) {
			ui_base_toggle_browser();
		}
		else if (keymap_shortcut(any_map_get(g_keymap, "toggle_2d_view"), SHORTCUT_TYPE_STARTED)) {
			ui_base_show_2d_view(VIEW_2D_TYPE_LAYER);
		}
	}

	if (keymap_shortcut(any_map_get(g_keymap, "file_save_as"), SHORTCUT_TYPE_STARTED)) {
		project_save_as(false);
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_save"), SHORTCUT_TYPE_STARTED)) {
		project_save(false);
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_open"), SHORTCUT_TYPE_STARTED)) {
		project_open();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_open_recent"), SHORTCUT_TYPE_STARTED)) {
		box_projects_show();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_reimport_mesh"), SHORTCUT_TYPE_STARTED)) {
		project_reimport_mesh();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_reimport_textures"), SHORTCUT_TYPE_STARTED)) {
		project_reimport_textures();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_new"), SHORTCUT_TYPE_STARTED)) {
		project_new_box();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_export_textures"), SHORTCUT_TYPE_STARTED)) {
	...
	else if (keymap_shortcut(any_map_get(g_keymap, "file_export_textures_as"), SHORTCUT_TYPE_STARTED)) {
		g_context->layers_export = EXPORT_MODE_VISIBLE;
		box_export_show_textures();
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "file_import_assets"), SHORTCUT_TYPE_STARTED)) {
		project_import_asset(NULL, true);
	}
	else if (keymap_shortcut(any_map_get(g_keymap, "edit_prefs"), SHORTCUT_TYPE_STARTED)) {
...
void util_shortcut_brush() {
	...
	bool adjusting =
	    keymap_shortcut(any_map_get(g_keymap, "brush_radius"), SHORTCUT_TYPE_DOWN) ||
	    keymap_shortcut(any_map_get(g_keymap, "brush_opacity"), SHORTCUT_TYPE_DOWN) ||
	    keymap_shortcut(any_map_get(g_keymap, "brush_angle"), SHORTCUT_TYPE_DOWN) ||
	    (decal_mask && keymap_shortcut(string_tmp("%s+%s", any_map_get(g_keymap, "decal_mask"), any_map_get(g_keymap, "brush_radius")), SHORTCUT_TYPE_DOWN));
	...
	if (keymap_shortcut(any_map_get(g_keymap, "brush_opacity"), SHORTCUT_TYPE_DOWN)) {
	...
	else if (keymap_shortcut(any_map_get(g_keymap, "brush_angle"), SHORTCUT_TYPE_DOWN)) {
		...
		make_material_parse_paint_material(true);
	         ...
	         keymap_shortcut(string_tmp("%s+%s", any_map_get(g_keymap, "decal_mask"), any_map_get(g_keymap, "brush_radius")), SHORTCUT_TYPE_DOWN)) {
...
void util_shortcut_viewport() {
	...
	if (!is_typing) {
		if (keymap_shortcut(any_map_get(g_keymap, "select_material"), SHORTCUT_TYPE_DOWN)) {
		...
		else if (keymap_shortcut(any_map_get(g_keymap, "select_layer"), SHORTCUT_TYPE_DOWN)) {
	...
	if (context_in_paint_area() && !is_typing) {
		...
		if (!mouse_down("right")) { // Fly mode off
			if (keymap_shortcut(any_map_get(g_keymap, "tool_brush"), SHORTCUT_TYPE_STARTED)) {
				context_select_tool(TOOL_TYPE_BRUSH);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_eraser"), SHORTCUT_TYPE_STARTED)) {
				context_select_tool(TOOL_TYPE_ERASER);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_fill"), SHORTCUT_TYPE_STARTED)) {
			...
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_text"), SHORTCUT_TYPE_STARTED)) {
				context_select_tool(TOOL_TYPE_TEXT);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_clone"), SHORTCUT_TYPE_STARTED)) {
				context_select_tool(TOOL_TYPE_CLONE);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_blur"), SHORTCUT_TYPE_STARTED)) {
			...
			else if (keymap_shortcut(any_map_get(g_keymap, "tool_select"), SHORTCUT_TYPE_STARTED)) {
				context_select_tool(TOOL_TYPE_SELECT);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "swap_brush_eraser"), SHORTCUT_TYPE_STARTED)) {
		...
		if (g_context->tool == TOOL_TYPE_BRUSH || g_context->tool == TOOL_TYPE_ERASER || g_context->tool == TOOL_TYPE_DECAL ||
			...
			if (keymap_shortcut(any_map_get(g_keymap, "brush_radius"), SHORTCUT_TYPE_STARTED) ||
			    keymap_shortcut(any_map_get(g_keymap, "brush_opacity"), SHORTCUT_TYPE_STARTED) ||
			    keymap_shortcut(any_map_get(g_keymap, "brush_angle"), SHORTCUT_TYPE_STARTED) ||
			    (decal_mask &&
			     keymap_shortcut(string_tmp("%s+%s", any_map_get(g_keymap, "decal_mask"), any_map_get(g_keymap, "brush_radius")), SHORTCUT_TYPE_STARTED))) {
				g_context->brush_locked = true;
				if (!pen_connected) {
					iron_mouse_lock();
				}
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "brush_radius_decrease"), SHORTCUT_TYPE_REPEAT)) {
			...
			else if (keymap_shortcut(any_map_get(g_keymap, "brush_radius_increase"), SHORTCUT_TYPE_REPEAT)) {
				g_context->brush_radius += ui_base_get_radius_increment();
				g_context->brush_radius   = math_round(g_context->brush_radius * 100) / 100.0;
				ui_header_handle->redraws = 2;
			}
		...
		if (decal_mask && (keymap_shortcut(any_map_get(g_keymap, "decal_mask"), SHORTCUT_TYPE_STARTED) ||
		                   keymap_shortcut(any_map_get(g_keymap, "decal_mask"), SHORTCUT_TYPE_RELEASED))) {
			ui_header_handle->redraws = 2;
		}
		...
		if (mouse_view_x() < sys_w()) {
			...
			else if (keymap_shortcut(any_map_get(g_keymap, "view_back"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(0, 1, 0, math_pi() / 2.0, 0, math_pi());
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_front"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(0, -1, 0, math_pi() / 2.0, 0, 0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_left"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(-1, 0, 0, math_pi() / 2.0, 0, -math_pi() / 2.0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_right"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(1, 0, 0, math_pi() / 2.0, 0, math_pi() / 2.0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_top"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(0, 0, 1, 0, 0, 0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_bottom"), SHORTCUT_TYPE_STARTED)) {
				viewport_set_view(0, 0, -1, math_pi(), 0, math_pi());
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_camera_type"), SHORTCUT_TYPE_STARTED)) {
			...
			else if (keymap_shortcut(any_map_get(g_keymap, "view_orbit_left"), SHORTCUT_TYPE_REPEAT)) {
				viewport_orbit(-math_pi() / 12.0, 0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_orbit_right"), SHORTCUT_TYPE_REPEAT)) {
				viewport_orbit(math_pi() / 12.0, 0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_orbit_up"), SHORTCUT_TYPE_REPEAT)) {
				viewport_orbit(0, -math_pi() / 12.0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_orbit_down"), SHORTCUT_TYPE_REPEAT)) {
				viewport_orbit(0, math_pi() / 12.0);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_orbit_opposite"), SHORTCUT_TYPE_STARTED)) {
				viewport_orbit_opposite();
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_zoom_in"), SHORTCUT_TYPE_REPEAT)) {
				viewport_zoom(0.2);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "view_zoom_out"), SHORTCUT_TYPE_REPEAT)) {
				viewport_zoom(-0.2);
			}
			else if (keymap_shortcut(any_map_get(g_keymap, "viewport_mode"), SHORTCUT_TYPE_STARTED)) {
